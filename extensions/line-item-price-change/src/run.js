...
function optionallyBuildUpdateOperation(
  { id: cartLineId, merchandise, cost, freeMealPlan }
) {
  console.log(freeMealPlan);
  const hasFreeMealPlan = freeMealPlan && freeMealPlan.value == "true";
  ...
  if (
    ...
    return {
      cartLineId,
      title: `${merchandise.product.title} - FREE MEAL PLAN(- ${cost.totalAmount.amount})`,
      price: {
        adjustment: {
          fixedPricePerUnit: {
            amount: 0
